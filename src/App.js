...
export default function App() {
  ...
  async function search(q) {
    ...
    const prompt = `You are a specialist in European exotic and collector car arbitrage with deep knowledge of global auction results, grey-market US imports, and the collector car market.

A user wants to find arbitrage opportunities for: "${searchQuery}"

Your job is to:
1. Identify what car this is (make/model/year/trim)
2. Research typical European asking prices (Germany, UK, France, Belgium, Netherlands, Italy, Switzerland)
3. Research typical US auction/resale prices (BaT, Mecum, RM Sotheby's, Broad Arrow, Gooding)
4. Calculate import costs to the US (the user will handle the math, just give prices)
5. Assess whether a genuine arbitrage opportunity exists

Important import context:
- Cars 25+ years old: EPA/DOT exempt (huge cost saving)
- Total duty burden from EU/UK: ~12.5% of purchase price (2.5% base + 10% reciprocal tariff)
- Plus ~$6,450 in fixed costs (shipping, broker, inland, prep)
- If car is under 25 years: add $15,000-35,000 for DOT/EPA compliance
...
Return ONLY a JSON object, no markdown, no explanation:
{
  "car": {
    ...
    "model": "<string>",
    "trim": "<string>",
    "production_total": <number or null>,
    "us_sold_new": <boolean>,
    "over_25_years": <boolean>,
    "description": "<2 sentences about what makes this car special>"
  },
  "european_market": {
    "typical_price_eur": <number>,
    "price_range_low_eur": <number>,
    "price_range_high_eur": <number>,
    "best_sources": ["<country/platform>", "<country/platform>", "<country/platform>"],
    "supply_notes": "<1 sentence about European supply/availability>"
  },
  "us_market": {
    "typical_auction_usd": <number>,
    "auction_range_low_usd": <number>,
    "auction_range_high_usd": <number>,
    "best_venues": ["<venue>", "<venue>"],
    "demand_notes": "<1 sentence about US demand>"
  },
  "arbitrage": {
    "opportunity_score": <0-100>,
    "verdict": "<Strong Buy | Moderate Opportunity | Marginal | Avoid>",
    "summary": "<3 sentences: what the opportunity is, why it exists, key risk>",
    "pros": ["<specific>", "<specific>", "<specific>"],
    "cons": ["<specific>", "<specific>", "<specific>"],
    "due_diligence": ["<specific check>", "<specific check>", "<specific check>", "<specific check>"]
  },
  "comparable_sales": [
    {"date": "<month year>", "venue": "<auction>", "price": "<$XXXk>", "notes": "<brief>"},
    {"date": "<month year>", "venue": "<auction>", "price": "<$XXXk>", "notes": "<brief>"},
    {"date": "<month year>", "venue": "<auction>", "price": "<$XXXk>", "notes": "<brief>"}
  ]
}`;
    ...
    try {
      ...
      setResult({ ...parsed, _costs: costs, _spread: spread, _priceUSD: priceUSD, _usAvg: usAvg, _query: searchQuery });
    } catch (err) {
      setError("Analysis failed. Make sure your ANTHROPIC_API_KEY is set in Vercel Environment Variables.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{ minHeight: "100vh", background: "#020817", color: "#f1f5f9", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 28px 0" }}>
        <div style={{ marginBottom: 12, fontSize: 13, color: "#64748b" }}>
          Enter any year, make, model, and trim to find European arbitrage opportunities
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <input
            ...
            style={{ flex: 1, padding: "12px 16px", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, color: "#f1f5f9", fontSize: 14, outline: "none", fontFamily: "'Inter', sans-serif" }}
        ...
        {error && (
          <div style={{ marginTop: 20, padding: 16, background: "#450a0a", border: "1px solid #7f1d1d", borderRadius: 8, fontSize: 12, color: "#fca5a5" }}>{error}</div>
        )}
        ...
        {result && !loading && (
          <div style={{ marginTop: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              <div>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, color: "#f8fafc", lineHeight: 1.2 }}>
            ...
            <button onClick={() => setExpandedCost(!expandedCost)}
              style={{ background: "none", border: "1px solid #1e293b", borderRadius: 6, color: "#475569", fontSize: 11, padding: "5px 12px", cursor: "pointer", marginBottom: expandedCost ? 0 : 16 }}>
              {expandedCost ? "▲ Hide cost breakdown" : "▼ Show full import cost breakdown"}
            </button>
            {expandedCost && (
              <div style={{ background: "#0a0f1a", border: "1px solid #1e293b", borderRadius: 8, padding: 14, marginBottom: 16, fontSize: 11 }}>
                {[
                  ["Purchase price (USD)", result._priceUSD],
                  ["Ocean freight (container)", IMPORT_COSTS.shipping],
                  ["Base duty 2.5%", result._costs?.duty],
                  ["EU reciprocal tariff 10%", result._costs?.recip],
                  ["HMF + MPF", (result._costs?.hmf || 0) + (result._costs?.mpf || 0)],
                ...
                {!result.car?.over_25_years && (
                  <div style={{ marginTop: 8, fontSize: 10, color: "#dc2626" }}>⚠ Car may be under 25 years — add $15–35k for DOT/EPA compliance</div>
                )}
              </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
              <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, padding: 14 }}>
                ...
                <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 8 }}>
                  Range: <span style={{ color: "#f1f5f9" }}>€{(result.european_market?.price_range_low_eur || 0).toLocaleString()} – €{(result.european_market?.price_range_high_eur || 0).toLocaleString()}</span>
                </div>
              ...
              <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, padding: 14 }}>
                ...
                <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 8 }}>
                  Range: <span style={{ color: "#f1f5f9" }}>{fmtK(result.us_market?.auction_range_low_usd || 0)} – {fmtK(result.us_market?.auction_range_high_usd || 0)}</span>
                </div>
            ...
            <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, padding: 14, marginBottom: 10, fontSize: 13, color: "#cbd5e1", lineHeight: 1.75 }}>
            ...
            <div style={{ background: "#150c00", border: "1px solid #78350f", borderRadius: 8, padding: 14, marginBottom: 10 }}>
              ...
              {result.arbitrage?.due_diligence?.map((d, i) => (
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 32 }}>
              <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && search()}
                ...
                style={{ flex: 1, padding: "10px 14px", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, color: "#f1f5f9", fontSize: 13, outline: "none", fontFamily: "'Inter', sans-serif" }}
                onFocus={e => e.target.style.borderColor = "#f59e0b"}
                onBlur={e => e.target.style.borderColor = "#1e293b"}
              />
              <button onClick={() => search()} disabled={loading || !query.trim()}
